...
def build_trigonal_planar_complex(db_path, central_atom, ligands):
    """
    Erstellt einen trigonal-planaren Komplex um das Zentralatom.
    """
    # Trigonal-planare Positionen relativ zum Ursprung
    positions = [
        np.array([2.0, 0.0, 0.0]),  # +x
        np.array([-1.0, np.sqrt(3), 0.0]),  # -x, +y
        np.array([-1.0, -np.sqrt(3), 0.0])  # -x, -y
    ]
...
def save_trigonal_planar_arrangement(db_path, central_atom, ox, ligands, output_dir, spin=None, chrg=0, mult=1):
    """
    Generiert einen einzigen trigonal-planaren Komplex und speichert ihn als .inp-Datei.
    """
    ...
    # Dateiname für den einzelnen Komplex
    file_name = f"{central_atom}_{ox}_{ligands_str}{spin_suffix}.inp"
    inp_file_path = os.path.join(complex_dir, file_name)

    # Baue den Komplex mit den Liganden in der gegebenen Reihenfolge
    complex_atoms = build_trigonal_planar_complex(db_path, central_atom, ligands)
...
if __name__ == "__main__":
    ...
    default_output_dir = os.path.join(parent_dir, "Komplexe", "Trigonal Planar")  # Ordner für trigonal-planare Geometrie

    # Argumentparser einrichten
    parser = argparse.ArgumentParser(description="Generiert trigonal-planare Komplexe mit Liganden.")
    ...
    if len(selected_names) != 3:
        raise ValueError("Es müssen genau 3 Liganden angegeben werden.")
    ...
    save_trigonal_planar_arrangement(db_path, central_atom, ox, selected_names, output_dir, spin, chrg, mult)